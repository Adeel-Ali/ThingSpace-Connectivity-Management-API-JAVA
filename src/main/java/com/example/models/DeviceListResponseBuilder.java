/*
 * ThingSpaceConnectivityManagementAPILib
 *
 * This file was automatically generated by APIMATIC v2.0 ( https://apimatic.io ) on 10/05/2016
 */
package com.example.models;

import java.util.*;

public class DeviceListResponseBuilder {
    //the instance to build
    private DeviceListResponse deviceListResponse;

    /**
     * Default constructor to initialize the instance
     */
    public DeviceListResponseBuilder() {
        deviceListResponse = new DeviceListResponse();
    }

    public DeviceListResponseBuilder devices(List<Device> devices) {
        deviceListResponse.setDevices(devices);
        return this;
    }

    public DeviceListResponseBuilder hasMoreData(Boolean hasMoreData) {
        deviceListResponse.setHasMoreData(hasMoreData);
        return this;
    }
    /**
     * Build the instance with the given values
     */
    public DeviceListResponse build() {
        return deviceListResponse;
    }
}